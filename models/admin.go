...
func AdminGetUserDetail(uid uint64) (err error, user User) {
	...
	user.GetUserDetail()
	if user.Uid == 0 {
		return util.NewError(util.ERROR_USER_NOT_EXIST), User{}
	}
	return
}
...
func AdminEditUserInfo(changedUid, operatorUid uint64, name, password, groupId, storage string) error {
	...
	if groupId != "0" && groupId != "" {
		// 判断指定的group_id是否存在
		// 取redis
		_, isExist := cache.GetRedisKey("groups_name_" + groupId)
		if isExist == false {
			return util.NewError(util.ERROR_GROUP_NOT_EXIST)
		}
		// 判定是否有权赋予这个groupId
...
func AdminGroupPermissionVerify(operatorUid uint64, groupPermission int64) error {
	...
	if err != nil || user.Uid == 0 || isExist == false {
		return util.NewError(util.ERROR_USER_NOT_EXIST)
	}
...
func AdminCreateGroup(operatorUid uint64, groupName string, groupPermission int64, groupStorageStr string) error {
	...
	cache.SetRedisKey("groups_name_"+groupID, newGroup.GroupName, 0)
	cache.SetRedisKey("groups_permission_"+groupID, newGroup.GroupPermission, 0)
	cache.SetRedisKey("groups_storage_"+groupID, newGroup.GroupStorage, 0)
	return nil
}
...
func AdminEditGroupInfo(operatorUid, groupId uint64, groupName string, groupPermission int64, groupStorage string) error {
	...
	if groupName != "" {
		...
		cache.SetRedisKey("groups_name_"+groupID, groupName, 0)
	}
	if groupPermission != 0 {
		changedInfo["group_permission"] = groupPermission
		cache.SetRedisKey("groups_permission_"+groupID, groupPermission, 0)
	}
	if groupStorage != "" {
		changedInfo["group_storage"] = groupStorage
		cache.SetRedisKey("groups_storage_"+groupID, groupStorage, 0)
	}
...
func AdminDeleteGroup(uid, groupID uint64) error {
	...
	if groupID == 1 || groupID == 2 {
		return util.NewError(util.ERROR_GROUP_DEFAULT)
	}
	...
	newGroupStorageStr, _ := cache.GetRedisKey("groups_storage_2")
	...
	cache.DelRedisKey("groups_permission_" + groupIDStr)
	cache.DelRedisKey("groups_name_" + groupIDStr)
	cache.DelRedisKey("groups_storage_" + groupIDStr)
	return nil
}
